...
def add_space(size="medium"):
    ...
    spaces = {"small": 1, "medium": 2, "large": 3}
...
def mood_form():
    ...
    with st.form("mood_form"):
        mood = st.slider(
            "Humeur générale (1-10)", 1, 10, 5
        )  # 5 par defaut c'est le milieu
        emotion = st.text_input(
            "💭 Ressenti émotionnel", placeholder="Ex: calme, anxieux, motivé..."
        )
        motivation = st.text_input(
            "🎯 Motivation du jour", placeholder="Ex: faire du sport, voir des amis..."
        )
        notes = st.text_area(
            "📝 Notes / Ressenti libre",
            placeholder="Tu peut écrire ce que tu veux ici...",
        )
...
def render_dashboard():
    ...
    if task_id and not st.session_state.get("notification_dismissed"):
        ...
        with col2:
            ...
            with col_a:
                if st.button(
                    "🎯 Lancer le timer", type="primary", use_container_width=True
                ):
            ...
            with col_b:
                if st.button("⏰ Rappeler dans 5 min", use_container_width=True):
                    ...
                    st.session_state.snooze_until = datetime.now() + timedelta(
                        minutes=5
                    )
                    st.rerun()
...
def render_focus_of_day():
    ...
    if task:
        ...
        # Bouton terminer
        if st.button(
            f"✅ Marquer '{title}' comme terminée", key=f"complete_focus_{task_id}"
        ):
...
def render_preferences_section():
    ...
    st.markdown("")
    if st.button(
        "💾 Sauvegarder les préférences", type="primary", use_container_width=True
    ):
...
def render_backup_management():
    ...
    with col2:
        ...
        if st.button(
            "📥 Créer un backup maintenant", type="primary", use_container_width=True
        ):
...
def render_export_section_styled():
    ...
    with col1:
        ...
        st.warning(
            "⚠️ **Note de confidentialité** : Tes données sont stockées localement et chiffrées sur ton appareil."
        )
...
def render_backups_section():
    ...
    with col2:
        ...
        st.caption(
            "Les backups sont créés au démarrage de l'application. Tu peux configurer le nombre de backups à conserver."
        )
...
def render_security_section_styled():
    ...
    with col1:
        st.markdown(
            "Cette section te permet de renforcer la sécurité de tes données en activant le **chiffrement**."
        )
    with col2:
    ...
    with col2:
        if is_enabled:
            ...
            # Bouton Activer
            if st.button(
                "🔒 Activer le chiffrement", type="primary", use_container_width=True
            ):
...
def render_task_calendar():
    ...
    with st.form("new_task_form", clear_on_submit=True):
        ...
        with col1:
            new_task = st.text_input(
                "", placeholder="Ajoute une tâche...", label_visibility="collapsed"
            )
        with col2:
            task_date = st.date_input(
                "📅", value=st.session_state.selected_day, label_visibility="collapsed"
            )
    ...
    tasks = get_today_tasks(
        task_date=st.session_state.selected_day, show_completed=False
    )
    ...
    if tasks:
        for task_id, title, done, time_spent, task_date_str, start_time in tasks:
            ...
            with col1:
                checked = st.checkbox(title, value=bool(done), key=f"task_{task_id}")
...
def render_task_history():
    ...
    if not df.empty:
        ...
        for date_value, group in grouped:
            ...
            st.markdown(
                f"**{completed}/{total} tâches terminées** | ⏱️ Temps total: {total_time:.1f}h"
            )

            for _, row in group.iterrows():
                status_icon = "✅" if row["done"] == 1 else "⬜"
                time_str = (
                    f"⏱️ {row['time_spent']:.1f}h" if row["time_spent"] > 0 else ""
                )
                completed_str = ""
                ...
                st.markdown(
                    f"{status_icon} **{row['title']}** {time_str}{completed_str}"
                )
...
def tasks_finish():
    ...
    df = pd.read_sql(
        ...
        FROM tasks 
        WHERE DATE(task_date) = ? AND done = 1
